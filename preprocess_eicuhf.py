...
#from cols_ import cols, medi, inputs, lab
...
data=pd.read_csv('.//mort48_inthour_eicu_hf.csv',index_col='patientunitstayid')
...
'''
medi=['captopril', 'metoprolol', 'atorvastatin',
       'simvastatin', 'aspirin', 'clopidogrel', 'bumetanide']
inputs=['nitroglycerin', 'diltiazem', 'esmolol', 'labetalol',
       'verapamil', 'metoprolol_i']
lab=['arterial_bp_systolic', 'arterial_bp_mean', 'heart_rate',
       'respiratory_rate', 'spo2', 'hemoglobin', 'hemoglobin_rate', 'glucose',
       'cholesterol', 'alt', 'ast', 'tg', 'tc', 'ldl', 'hdl', 'ck', 'tni',
       'tnt']
'''
def str2date(date):
...
for p in pbar:
    ...
    if len(temp.shape)==2:
        ...
        temp.to_csv('.//mortality48_hour_eicu_hf//'+str(p)+'.csv')
...
data_sta.to_csv('.//mortality48_hour_eicu_hf//data_info//data_info.csv')
...
train.to_csv('.//mortality48_hour_eicu_hf//data_info//train.csv')
test.to_csv('.//mortality48_hour_eicu_hf//data_info//test.csv')
...
for i in range(fold_num-1):
    ...
    fold.to_csv('.//mortality48_hour_eicu_hf//data_info//fold'+str(i)+'_info.csv')
    folds.append(fold)
...
fold.to_csv('.//mortality48_hour_eicu_hf//data_info//fold'+str(fold_num-1)+'_info.csv')
folds.append(fold)
...
folds_info.to_csv('.//mortality48_hour_eicu_hf//data_info//folds_info.csv')
